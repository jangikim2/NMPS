...
@hydra.main(config_path='.', config_name='exploration')
def exploration_cfg_read(cfg):
    while 1:
        print(cfg)
...
class Workspace:
    def __init__(self, cfg):
        ...
        print("self.train_env.observation_spec() : ", self.train_env.observation_spec())
        print("self.train_env.action_spec() : ", self.train_env.action_spec())
        ...
        ## exploration_cfg_read()

        agent_dir = self.work_dir / Path(self.cfg.agent_dir)
        ##exploration_yaml = agent_dir / f'diayn.yaml'
        exploration_yaml = agent_dir / self.cfg.exploration_yaml
        ##while 1:
        ##    print("exploration_yaml : ", exploration_yaml)

        ##conf = OmegaConf.load(exploration_yaml)
        #hydra.core.global_hydra.GlobalHydra.instance().clear()
        #with initialize('.'):
        ##initialize('../../../')
        ##initialize_config_dir(agent_dir)
        #    self.conf = compose(config_name='exploration')
        #while 1:
        #    print("cfg : ", cfg)
        #    print("conf : ", conf)
        #    print("data.agent : ", cfg.agent)
        #    print("exploration_data.agent : ", conf.agent)
        ...
        #with open(f'exploration_yaml') as f:
        ####with exploration_yaml.open('r') as f:
            ##exploration_data = yaml.load(f, Loader=SafeLoader)
        ####    exploration_data = yaml.load(f, Loader=yaml.FullLoader)

        ####    while 1:
        ####        print(cfg)
        ####        print(exploration_data)
        ####        print("exploration_data.agent : ", exploration_data['defaults']['agent'])

        ##exploration_data.obs_type = cfg.obs_type
        ##exploration_data.update({'agent': f'cfg.exploration_agent'})

        #agent = cfg.exploration_agent
        #print("agent ", agent)
        #exploration_data["agent"] = eval("agent")

        ##while 1:
        ##    print('cfg.agent ', cfg.agent)
        #print("cfg.obs_type ", cfg.obs_type)
        #print(exploration_data)
        #print("exploration_data.agent ", exploration_data['name'])
        ##while 1:
        ##    print(f"Current working directory : {os.getcwd()}")
            #print('cfg.obs_type ', cfg.obs_type)
            #print('self.train_env.observation_spec() ', self.train_env.observation_spec())
            #print('self.train_env.action_spec() ', self.train_env.action_spec())
            #print('cfg.num_seed_frames // cfg.action_repeat ', cfg.num_seed_frames // cfg.action_repeat)
            #print("conf.agent ", conf.agent)
        ...
        with initialize('.'):
            ...
            # self.agent_exploration = make_agent(self.cfg.obs_type,
            #                                     self.train_env.observation_spec(),
            #                                     self.train_env.action_spec(),
            ...
            ########## agent_exploration
            self.agent_exploration = make_agent(self.conf.obs_type,
                                                self.train_env.observation_spec(),
                                                self.train_env.action_spec(),
                                                self.conf.num_seed_frames // self.conf.action_repeat,
                                                self.conf.agent)

        ########## agent_exploration
        meta_exploration_specs = self.agent_exploration.get_meta_specs()

        ########## agent_exploration
        self.replay_storage_exploration = ReplayBufferStorage(data_specs, meta_exploration_specs,
                                                  self.work_dir / 'buffer_explorations')
												  
        ########## agent_exploration
        # self.replay_loader_exploration = make_replay_loader(self.replay_storage_exploration,
        #                                         cfg.replay_buffer_size,
        #                                         cfg.batch_size,
        #                                         cfg.replay_buffer_num_workers,
        #                                         False, cfg.nstep, cfg.discount)
        self.replay_loader_exploration = make_replay_loader(self.replay_storage_exploration,
                                                self.conf.replay_buffer_size,
                                                self.conf.batch_size,
                                                self.conf.replay_buffer_num_workers,
                                                False, self.conf.nstep, self.conf.discount)
        ########## agent_exploration
        self._replay_iter_exploration = None												  
    ...
    ########## agent_exploration
    @property
    def replay_iter_exploration(self):
        if self._replay_iter_exploration is None:
            self._replay_iter_exploration = iter(self.replay_loader_exploration)
        return self._replay_iter_exploration
    ...
    def train(self):
        ...
        meta = self.agent.init_meta()
        ########## agent_exploration
        meta_exploration = self.agent_exploration.init_meta()
        self.replay_storage.add(time_step, meta)
        ########## agent_exploration
        self.replay_storage_exploration.add(time_step, meta_exploration)
        ...
        while train_until_step(self.global_step):
            ...
            ########## agent_exploration
            meta_exploration = self.agent_exploration.update_meta(meta_exploration, self.global_step, time_step)
            #####################################################################
            ...
            # sample action
            if self.action_m == 1:
                #print("self.action_m : ", self.action_m)
                with torch.no_grad(), utils.eval_mode(self.agent):
            ...
            if not seed_until_step(self.global_step):
                ...
                if self.global_step % self.agent.update_every_steps != 0:
                    ...
                    ########## agent_exploration
                    metrics = self.agent_exploration.update_pretraining_first(self.replay_iter_exploration, self.global_step)
                    #metrics = self.agent_exploration.update(self.replay_iter, self.global_step)
                    ...
                    ########## agent_exploration
                    #replay_buffer_obs, replay_buffer_action, replay_buffer_extr_reward, replay_buffer_discount, replay_buffer_next_obs, replay_buffer_task, first_metrics = self.agent_exploration.update_pretraining_first(self.replay_iter, self.global_step)
                    replay_buffer_obs, replay_buffer_action, replay_buffer_extr_reward, replay_buffer_discount, replay_buffer_next_obs, replay_buffer_task, first_metrics = self.agent_exploration.update_pretraining_first(self.replay_iter_exploration, self.global_step)
                    self.agent_exploration.direct_update_diayn(replay_buffer_task, replay_buffer_next_obs, self.global_step)
                    ###metrics = self.agent_exploration.update_pretraining_second(self.replay_iter,
                    ########## agent_exploration
                    #metrics = self.agent_exploration.update_pretraining_second(self.replay_iter,
                    metrics = self.agent_exploration.update_pretraining_second(self.replay_iter_exploration,
                                                                               self.global_step,
                                                                               replay_buffer_obs,
                                                                               replay_buffer_action,
                                                                               replay_buffer_extr_reward,
