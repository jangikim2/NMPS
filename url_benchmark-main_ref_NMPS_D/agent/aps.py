...
class APSAgent(DDPGAgent):
    ...
    def compute_intr_ent_reward(self, task, next_obs, step):
        # maxent reward
        with torch.no_grad():
            rep = self.aps(next_obs, norm=False)
        reward = self.pbe(rep)
    ...
    def compute_intr_sf_reward(self, task, next_obs, step):
        # maxent reward
        with torch.no_grad():
            rep = self.aps(next_obs, norm=False)
        ##reward = self.pbe(rep)
    ...
    def direct_update_aps(self, task, next_obs, step):
        metrics = dict()
    ...
    def update_pretraining_first(self, replay_iter, step):
        #print("update_pretraining_first_start")
        metrics = dict()
        #print("update_pretraining_first_start_000")
        if step % self.update_every_steps != 0:
            return metrics
        #print("update_pretraining_first_start_0")
        batch = next(replay_iter)
        #print("update_pretraining_first_start_1")
        obs, action, extr_reward, discount, next_obs, task = utils.to_torch(
            batch, self.device)
        #print("update_pretraining_first_start_2")
        # augment and encode
        obs = self.aug_and_encode(obs)
        next_obs = self.aug_and_encode(next_obs)
        #print("update_pretraining_first_start_3")
        #####################################################################
        #metrics.update(self.update_aps(task, next_obs, step))
        #print("update_pretraining_first_end")
        return obs, action, extr_reward, discount, next_obs, task, metrics
        #####################################################################
        #if self.reward_free:
            # freeze successor features at finetuning phase
            # metrics.update(self.update_aps(task, next_obs, step))
    ...
    def update_pretraining_second(self,
                                  replay_iter,
                                  step,
                                  obs,
                                  action,
        ...
        #####################################################################
        #print("update_pretraining_second_start")
        #metrics.update(self.update_aps(task, next_obs, step))
        ...
        metrics['exploit_count'] = exploit_count
        metrics['explore_count'] = explore_count

        ##reward = intr_reward
        if SF_train_flag == True:
            reward = intr_sf_reward
        else:
        ...
        #print("update_pretraining_second_end")
        return metrics
    ...
    ##def read_target_value(self, replay_iter, step):
    def read_target_value(self, obs, meta, step):
        ...
        next_obs = torch.cat(inputs, dim=-1)
        ...
        with torch.no_grad():
            ...
            #                                          task)
            #print("meta : ", meta)
            #print("meta['task'] : ", meta['task'])
            #print("torch.tensor(meta['task']) : ", torch.tensor(meta['task']))
            target_Q1, target_Q2 = self.critic_target(next_obs, next_action,
                                                      torch.as_tensor([meta['task']],device=self.device))
